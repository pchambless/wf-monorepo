 ...
 * SelectVals Loader
 *
 * Loads selectVals.json data in a way that's compatible with both
 * Node.js and browser/React build environments
 */
...
if (isNode) {
  // In Node.js, try to load from file system
  try {
    // Use require for Node.js compatibility (this file will be transpiled)
    const fs = require("fs");
    ...
    const selectValsPath = path.join(__dirname, "selectVals.json");
    selectVals = JSON.parse(fs.readFileSync(selectValsPath, "utf8"));
  } catch (error) {
    console.warn(
      "Failed to load selectVals.json in Node.js, using fallback data:",
      error.message
    );
  }
}

// If not loaded from file system or in browser, use inline data
if (!selectVals || Object.keys(selectVals).length === 0) {
  selectVals = {
    registeredWorkflows: {
    ...
    // Provide empty structures for other config sections that might be needed
