CREATE
  ...
  SELECT
    `a`.`prodTypeName` AS `prodTypeName`,`a`.`prodName` AS `prodName`,`a`.`prodCode` AS `prodCode`,`a`.`prodBtchDate` AS `prodBtchDate`,`a`.`prodBtchNbr` AS `prodBtchNbr`,`a`.`account_id` AS `account_id`,`a`.`product_id` AS `product_id`,`a`.`product_batch_id` AS `product_batch_id`
