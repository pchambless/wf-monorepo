...
SELECT
p.package AS pPackage,
c.package AS cPackage,
(case when ((p.active = 1)
AND (c.active = 1)) then 'Valid' else 'Obsolete' end) AS status,
p.fileName AS pFile,
p.module_type AS pType,
c.fileName AS cFile,
c.module_type AS cType,
p.file_path AS pPath,
c.file_path AS cPath,
...
p.last_detected_at AS pDetected_at,
c.last_detected_at AS cDetected_at
FROM ((module_xref a
