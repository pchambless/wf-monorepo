...
LEFT JOIN (select whatsfresh.product_recipes.ingredient_id AS ingredient_id,
count(0) AS rcpeCnt from whatsfresh.product_recipes
WHERE (whatsfresh.product_recipes.active = 'Y')
GROUP BY whatsfresh.product_recipes.ingredient_id) rcpe on((a.ingredient_id = rcpe.ingredient_id))) group by a.ingrTypeName,
a.ingrName,
a.ingrCode,
a.ingrDesc,
a.gmsPerOz,
