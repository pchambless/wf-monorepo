...
AND (module_xref.deleted_at is null))) AS depends_on_count,
(select count(0) from module_xref where ((module_xref.module_id = m.id)
AND (module_xref.deleted_at is null))) AS depended_by_count,
t.id AS impact_id,
...
AND (t.id is null)) then 'Active (undocumented)' else 'Active (documented)' end) AS audit_status,
(case when m.package in (select distinct modules.package from modules where (modules.active = 1)) then 0 else 1 end) AS orphan_package from ((modules m
