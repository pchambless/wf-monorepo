...
ifnull(e.task_maps,
0) AS taskMaps,
ifnull((d.ingr_maps + e.task_maps),
0) AS totalMaps,
ifnull(pb.location,
'') AS location,
ifnull(pb.batch_quantity,
0) AS btchQty,
ifnull(whatsfresh.f_measure(pb.measure_id),
'') AS btchMeas,
ifnull(concat(pb.batch_quantity,
' ',
whatsfresh.f_measure(pb.measure_id),
's'),
'') AS qtyMeas,
ifnull(cast(date_format(pb.batch_start,
'%Y-%m-%d') as date),
'') AS prodBtchDate,
ifnull(pb.comments,
'') AS comments,
ifnull(pb.batch_number,
'No Batches') AS prodBtchNbr,
ifnull(date_format(pb.best_by_date,
'%Y-%m-%d'),
'') AS bestByDate,
...
LEFT JOIN (select whatsfresh.product_batch_ingredients.product_batch_id AS product_batch_id,
count(0) AS ingr_maps from whatsfresh.product_batch_ingredients
GROUP BY whatsfresh.product_batch_ingredients.product_batch_id) d on((pb.id = d.product_batch_id)))
LEFT JOIN (select whatsfresh.product_batch_tasks.product_batch_id AS product_batch_id,
count(0) AS task_maps from whatsfresh.product_batch_tasks group by whatsfresh.product_batch_tasks.product_batch_id) e on((pb.id = e.product_batch_id)))
