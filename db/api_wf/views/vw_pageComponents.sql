...
pc.pageID AS pageID,
pc.comp_name AS comp_name,
pc.composite_id AS composite_id,
pc.parent_id AS parent_id,
pc.posOrder AS posOrder,
substring_index(pc.posOrder,
',
',
1) AS row_pos,
substring_index(substring_index(pc.posOrder,
',
',
2),
',
...
c.title AS composite_title,
c.components AS composite_components,
ve.qryName AS qryName,
ve.qrySQL AS qrySQL,
ve.props AS props,
ve.triggers AS triggers,
(case when (c.name in ('Form',
'ModalForm')) then 'form' when (c.name = 'Grid') then 'grid' else NULL end) AS field_generator_type
