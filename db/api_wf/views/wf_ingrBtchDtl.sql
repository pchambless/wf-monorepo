...
i.name AS ingrName,
i.code AS ingrCode,
ifnull(ib.batch_number,
'No Batches') AS ingrBtchNbr,
i.grams_per_ounce AS gmsPerOz,
i.description AS ingrDesc,
i.default_measure_unit AS dfltMeasId,
i.default_vendor AS dfltVndrId,
ifnull(ib.lot_number,
'') AS ingrLotNbr,
(ifnull(date_format(ib.purchase_date,
'%Y-%m-%d'),
'') collate utf8mb4_unicode_ci) AS purchDate,
ifnull(ib.purchase_quantity,
'') AS purchQty,
whatsfresh.f_measure(ib.measure_id) AS purchMeas,
ib.unit_quantity AS unitQty,
ib.unit_price AS unitPrice,
cast((ifnull(ib.unit_price,
0) / ifnull(ib.unit_quantity,
1)) as decimal(9,
4)) AS unitRate,
ifnull(concat(ib.purchase_quantity,
' @ ',
concat('$',
format(ifnull(ib.unit_price,
0),
2)),
' per ',
(trim(round(ib.unit_quantity,
2)) + 0),
' ',
whatsfresh.f_measure(ib.measure_id)),
'') AS purchDtl,
concat('$',
format(ifnull((ib.purchase_quantity * ib.unit_price),
0),
2)) AS purchAmt,
ifnull((ib.purchase_quantity * ib.unit_price),
0) AS purchTotalAmt,
whatsfresh.f_vendor(ib.vendor_id) AS vndrName,
whatsfresh.f_brand(ib.brand_id) AS brndName,
ifnull(date_format(ib.best_by_date,
'%Y-%m-%d'),
'') AS bestByDate,
ifnull(pbi.prd_btch_cnt,
0) AS prdBtchCnt,
ib.comments AS comments,
concat(ib.purchase_date,
' : ',
whatsfresh.f_vendor(ib.vendor_id)) AS shopEvent,
...
a.active AS active from whatsfresh.ingredient_batches a
...
LEFT JOIN (select whatsfresh.product_batch_ingredients.ingredient_batch_id AS ingredient_batch_id,
count(0) AS prd_btch_cnt from whatsfresh.product_batch_ingredients
