...
function generateMermaid(hierarchyData) {
  ...
  filteredData.forEach(item => {
    ...
    const nodeLabel = `${nodeId}<br/>ctgry: [${item.comp_type.toLowerCase()}]`;
    lines.push(`    ${nodeId}["${nodeLabel}"]`);

    const parent = filteredData.find(p => p.xref_id === item.parent_id);
    if (parent) {
  ...
  filteredData.forEach(item => {
    lines.push(`    click ${item.comp_name} href "javascript:window.selectEventTypeTab(${item.xref_id})"`);
  });
...
async function buildPageConfig(hierarchyData, userEmail = 'pc7900@gmail.com') {
    ...
    return hierarchyData
      ...
      .map(item => {
        ...
        if (item.enhancedProps) {
          props = typeof item.enhancedProps === 'string' ? JSON.parse(item.enhancedProps) : item.enhancedProps;
        } else if (item.props) {
        ...
        // Parse override_styles (base styles come from eventType templates in DirectRenderer)
        let overrideStyles = null;
        try {
          if (item.override_styles) {
            overrideStyles = JSON.parse(item.override_styles);
          }
        } catch (error) {
          logger.error(`${codeName} Error parsing override_styles for ${item.comp_name}:`, error);
        }

        // Get existing child components first
        const existingChildren = hierarchyData.some(child => child.parent_id === item.xref_id) ?
          buildChildren(item.xref_id, level + 1) : [];
        ...
        if (item.comp_type === 'Form' && cleanProps.fields && Array.isArray(cleanProps.fields)) {
        ...
        if (item.comp_type === 'Grid' && cleanProps.columns && Array.isArray(cleanProps.columns)) {
          ...
          const tableComponent = {
            ...
            components: [
              ...
              // Table body (will be populated by data)
              {
                ...
                type: 'tbody',
                props: {
                  dataSource: item.comp_name, // Use grid's component ID to find data in dataStore
                ...
                components: [
                  {
                    id: 'placeholderRow',
                    type: 'tr',
                    style: {}, // No background - will be set dynamically for alternating rows
                    components: props.columns.map(col => ({
                      id: `placeholder_${col.field}`,
                      type: 'td',
                      textContent: `{${col.field}}`,
                      style: {
                        padding: '6px 12px', // Compact: reduced vertical padding
  ...
  return {
    layout: "flex",
    ...(rootComponent.workflowTriggers && { workflowTriggers: rootComponent.workflowTriggers }),
    components: buildChildren(rootComponent.xref_id, 1),
...
export async function genPageConfig(pageID) {
  ...
  try {
    ...
    try {
      // Convert hyphenated page name to valid JavaScript identifier (PascalCase)
      const componentName = pageName
        .split('-')
        .map(part => part.charAt(0).toUpperCase() + part.slice(1))
        .join('');

      // Generate generic index.jsx template with dev mode auto-reload
...
const PAGE_ID = ${pageID}; // ${pageName} xref ID
