  ...
  if (fs.existsSync(registryPath)) {
    ...
    registry = {
      metadata: {
        created: today,
        description:
          "Master registry of all development plans with ID-to-name mapping",
        version: "1.0",
...
function createPlanFile(planId, cluster, planName) {
  const fileName = `${planId}-${cluster}-${planName
...
function updateImpactTracking(planId, cluster, planName) {
  ...
  // Add initial placeholder impact entry (using sequential numeric ID)
  const newImpact = {
    plan_id: planId, // Only the ID, not the full name
    impact_id: getNextImpactId(json),
    package: "TBD",
    file: "TBD",
    type: "ANALYZED",
    status: "pending",
...
function main() {
  ...
  try {
    ...
    console.log(
      `🎯 Cluster: ${cluster} (${VALID_CLUSTERS[cluster]} blast radius)`
    );
    ...
    console.log(
      `2. Add specific impacts to impact-tracking.json (reference plan_id: "${planId}")`
    );
    console.log(`3. Update predicted file counts in the plan`);
    console.log("");
    console.log(`Query examples:`);
    console.log(
      `# Get plan name: jq '.plans[] | select(.id == "${planId}") | .name' plan-registry.json`
    );
    console.log(
      `# Get plan impacts: jq '.impacts[] | select(.plan_id == "${planId}")' impact-tracking.json`
    );
  } catch (error) {
